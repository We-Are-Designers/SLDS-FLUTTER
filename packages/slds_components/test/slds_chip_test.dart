...
void main() {
  ...
  testWidgets('shows the close icon only when onDeleted is set', (tester) async {
    await pump(tester, const SldsChip(label: 'Label'));
  ...
  testWidgets('tapping the close icon fires onDeleted, not onTap', (tester) async {
    var deleted = false;
    var tapped = false;
    await pump(
      tester,
  ...
  testWidgets('avatar takes precedence over icon', (tester) async {
    await pump(
      tester,
      const SldsChip(label: 'Label', avatar: SldsAvatar(initials: 'LK'), icon: Icons.add),
    );
