...
void main() {
  ...
  testWidgets('renders illustration, title and description', (tester) async {
    ...
    expect(
      find.text('Your uploaded documents will appear here once added.'),
      findsOneWidget,
    );
  });
  ...
  testWidgets('hides description when null', (tester) async {
    await pump(
      tester,
      const SldsEmptyState(illustration: illustration, title: 'Title'),
    );
    expect(find.text('Title'), findsOneWidget);
  });

  testWidgets(
    'shows the action button only when both label and callback are set',
    (tester) async {
      await pump(
        tester,
        const SldsEmptyState(illustration: illustration, title: 'Title'),
