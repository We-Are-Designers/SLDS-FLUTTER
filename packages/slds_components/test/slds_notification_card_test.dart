...
void main() {
  ...
  testWidgets('renders title, body and timestamp', (tester) async {
    ...
    expect(find.text('Download it now or collect it from your selected office.'), findsOneWidget);
  ...
  testWidgets('shows the action button only when actionLabel is set, and it invokes onAction', (
    tester,
  ) async {
    var tapped = false;
    await pump(
  ...
  testWidgets('action button renders without crashing at a mobile viewport width', (tester) async {
  ...
  testWidgets('no action button when actionLabel is null', (tester) async {
    await pump(tester, const SldsNotificationCard(title: 'Title', body: 'Body'));
    expect(find.byType(SldsButton), findsNothing);
  });

  testWidgets('shows the unread dot only when unread is true', (tester) async {
    await pump(tester, const SldsNotificationCard(title: 'Title', body: 'Body'));
    expect(find.byWidgetPredicate((w) => w is Container && w.constraints?.maxWidth == 8), findsNothing);

    await pump(tester, const SldsNotificationCard(title: 'Title', body: 'Body', unread: true));
    // Presence check via the widget tree: a Container sized 8x8 (the dot).
    final dot = find.byWidgetPredicate((w) => w is Container && w.constraints == const BoxConstraints.tightFor(width: 8, height: 8));
  ...
  testWidgets('each type renders its own icon', (tester) async {
    for (final (type, icon) in [
      ...
      await pump(tester, SldsNotificationCard(title: 'Title', body: 'Body', type: type));
  ...
  testWidgets('swiping left past the threshold with onDismissed set calls the callback', (
    tester,
  ) async {
