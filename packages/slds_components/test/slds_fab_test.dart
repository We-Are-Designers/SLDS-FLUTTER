...
void main() {
  ...
  testWidgets('primary and destructive variants render filled', (tester) async {
    ...
    final primary = tester.widget<FloatingActionButton>(find.byType(FloatingActionButton));
    expect(primary.backgroundColor, isNot(Colors.transparent));

    await pump(
      tester,
      SldsFab(icon: Icons.delete, variant: SldsButtonVariant.destructive, onPressed: () {}),
    );
    expect(find.byType(FloatingActionButton), findsOneWidget);
  });

  testWidgets('secondary variant renders outlined on surface', (tester) async {
    await pump(tester, SldsFab(icon: Icons.add, variant: SldsButtonVariant.secondary, onPressed: () {}));
  ...
  testWidgets('badgeCount shows a numeric badge', (tester) async {
    await pump(tester, SldsFab(icon: Icons.add, badgeCount: 3, onPressed: () {}));
